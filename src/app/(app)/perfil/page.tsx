...
export default function PerfilPage() {
  ...
  const colportores = getDB().usuarios.filter((u) => u.rol === "colportor");
  ...
  return (
    <div className="px-5 pt-6">
      <header className="card flex items-center gap-4 p-5">
        ...
        <div className="flex-1">
          ...
          <div className="mt-1 flex items-center gap-2">
            ...
            >
              {session.esMonitor ? "Monitor" : "Colportor"}
            </span>
            {(session.user.ciudad || session.user.departamento) && (
              <span className="flex items-center gap-1 text-xs text-(--color-fg-muted)">
                <MapPin size={12} />
                {session.user.ciudad}
                {session.user.departamento ? `, ${session.user.departamento}` : ""}
              </span>
            )}
          </div>
        </div>
      ...
      {session.esMonitor && colportores.length > 0 && (
      ...
      <section className="mt-6">
        ...
        >
          ...
          <span className="text-sm font-medium text-(--color-error)">Cerrar sesión</span>
        </button>
      </section>
